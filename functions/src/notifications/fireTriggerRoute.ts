...
const KIND_AUDIENCE: Record<string, AudienceKind> = {
  ...
  // Designer Portal — design review loop
  design_review_requested: 'client_of_project',
  design_client_decided: 'staff_on_project',
};
...
export function registerFireTriggerRoute(
  app: Express,
  ...
  app.post('/api/notifications/fire', async (req: any, res: any) => {
    ...
    const singleUidOverride: string | undefined =
      Array.isArray(overrides.targetUserIds) &&
      overrides.targetUserIds.length === 1
    ...
    if (audKind === 'single_uid') {
      ...
      if (
        !Array.isArray(overrides.targetUserIds) ||
        overrides.targetUserIds.length === 0
      ) {
        return res
          .status(400)
          .json({ error: `Kind ${kind} requires overrides.targetUserIds` });
      }
      recipients = overrides.targetUserIds.map(String);
    } else if (audKind) {
      recipients = await resolveAudience(db, audKind, {
        projectId,
        singleUid: singleUidOverride,
      });
    } else {
    ...
    if (recipients.length === 0) {
      ...
      console.warn(
        '[fireTriggerRoute] empty audience for',
        kind,
        'projectId=',
        projectId
      );
    ...
    const fromUserName =
      (req.userProfile?.displayName as string | undefined) ||
