...
import { StyleProfileCard } from '@/components/designer/StyleProfileCard';
import {
  CheckCircle,
  AlertTriangle,
  Clock,
...
export function ProjectDesignDashboard({
  ...
  const totalApproved = selections.filter(
  ...
  const urgentSelections = [...selections]
    ...
    .sort((a, b) => {
      const aOver = isOverdue(a)
        ...
        : isDueSoon(a)
          ? 1
          : a.required !== false && a.items.length === 0
            ? 2
            : 3;
      const bOver = isOverdue(b)
        ...
        : isDueSoon(b)
          ? 1
          : b.required !== false && b.items.length === 0
            ? 2
            : 3;
  ...
  if (lastLoginAt) {
    for (const sel of selections) {
      const updatedAt = sel.updatedAt?.toDate
        ? sel.updatedAt.toDate()
        : sel.updatedAt
          ? new Date(sel.updatedAt)
          : null;
      ...
      for (const comment of sel.comments || []) {
        const commentAt = comment.createdAt?.toDate
        ...
        if (commentAt && commentAt > lastLoginAt) {
          const locLabel =
            sel.roomName || roomMap[sel.roomId || ''] || sel.room;
          activityEvents.push({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <div className="text-xs text-gray-400 mt-1">
            {totalApproved} of {totalRequired} required
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          ...
          <div className="text-xs text-gray-400 mt-1">
            approved / {selections.length} total
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center justify-between mb-1">
            ...
            <AlertTriangle
              className={`w-4 h-4 ${overdueCount > 0 ? 'text-red-500' : 'text-gray-300'}`}
            />
          </div>
          <div
            className={`text-3xl font-bold ${overdueCount > 0 ? 'text-red-600' : 'text-gray-900'}`}
          >
            {overdueCount}
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center justify-between mb-1">
            ...
            <MessageSquare
              className={`w-4 h-4 ${openQuestions > 0 ? 'text-amber-500' : 'text-gray-300'}`}
            />
          </div>
          <div
            className={`text-3xl font-bold ${openQuestions > 0 ? 'text-amber-600' : 'text-gray-900'}`}
          >
            {openQuestions}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {categoryStats.length === 0 ? (
            ...
            <div className="space-y-3">
              {categoryStats.map(({ cat, total, approved, hasOverdue }) => {
                ...
                const barColor =
                  pct === 100 ? '#22c55e' : hasOverdue ? '#ef4444' : '#C9A96E';
                return (
                  <div key={cat}>
                    <div className="flex items-center justify-between mb-1">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h3 className="text-sm font-semibold text-gray-800 mb-4">
            Next Crucial Selections
          </h3>
          {urgentSelections.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <CheckCircle className="w-8 h-8 text-green-400 mb-2" />
              <p className="text-sm text-gray-500">
                All required selections approved!
              </p>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                      {dd && (
                        <span
                          className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs font-medium ${dd.color}`}
                        >
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          {lastLoginAt && (
            <p className="text-xs text-gray-400 mb-4">
              Since {lastLoginAt.toLocaleDateString()}{' '}
              {lastLoginAt.toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit',
              })}
            </p>
          ...
          {!lastLoginAt && (
            <p className="text-xs text-gray-400 mb-4">
              First visit — showing recent activity
            </p>
          )}
      ...
      {/* Client's blind Style Discovery result — what they actually leaned toward */}
      <StyleProfileCard projectId={projectId} />
