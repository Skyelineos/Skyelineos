...
**Claude runs the git, not Tyler.** Tyler does not work through the terminal. Every session **handles its own version control automatically** — stage, commit with a clear message, and `git push` to the working branch, then open a PR — **without asking Tyler to run any git commands** (this is durable, standing authorization to commit + push). Surface the PR so Tyler can review; **merging deploys to production**, so merge on his go-ahead (he can also say "merge it" / "auto-merge" to skip that step). Never end a session with finished work left uncommitted or unpushed. If a git step genuinely can't be done from the session's environment (e.g. branch deletion blocked by the remote proxy), do the rest and hand Tyler a single paste-ready command rather than a terminal workflow.
